...
class FL_env():
    ...
    def reset(self):
        ...
        # client normalize
        num_client_n = (f.num_clients - 1) / (f.max_slicing - 1)
    ...
    def step(self, round, action, agent, pre_train, last_slicing):
        ...
        reward = (good_to_good * ((1 - 0.4) / (1 - f.attack_ratio)) + bad_to_bad * (0.4 / f.attack_ratio)) - math.log(51 - last_slicing)
        '''
        if intermediate_u > 0:
        ...
        if round >= 20:
            ...
            num_client_n = (action[2] - 1) / (f.max_slicing - 1)
