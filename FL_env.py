        ...
        for id_c in self.my_shuffle.shuffle_in_bad:
            for id_u in self.my_clients[id_c].local_users:
                if id_u in self.my_attackers.all_attacker:
                    ...
                    good_to_bad += 1
        # reward function
        reward = (good_to_good * ((1 - 0.4) / (1 - f.attack_ratio)) + bad_to_bad * (0.4 / f.attack_ratio) - good_to_bad * ((1 - 0.4) / (1 - f.attack_ratio)) - bad_to_good * (0.4 / f.attack_ratio)) * (0.999 ** math.log(action[2]))
